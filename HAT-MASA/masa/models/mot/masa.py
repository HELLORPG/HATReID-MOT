            ...
            if self.load_public_dets:
                ...
                if img_name is not None:
                    if self.benchmark == "bdd":
                        ...
                        pickle_name = img_name.replace("data/dancetrack/val/", "").replace(
                            ".jpg", self.end_pkl_name
                        )
                    elif self.benchmark == "dancetrack_test":
                        pickle_name = img_name.replace("data/dancetrack/test/", "").replace(
                            ".jpg", self.end_pkl_name
                        )
                    elif self.benchmark == "mot17":
                        pickle_name = img_name.replace("data/mot/test/", "").replace(
                            ".jpg", self.end_pkl_name
                        )
                    elif self.benchmark == "mot17_half":
                        pickle_name = img_name.replace("data/mot/train/", "").replace(
                            ".jpg", self.end_pkl_name
                        )
                    elif self.benchmark == "sportsmot":
                        pickle_name = img_name.replace("data/sportsmot/val/", "").replace(
                            ".jpg", self.end_pkl_name
                        )
                    elif self.benchmark == "sportsmot_test":
                        pickle_name = img_name.replace("data/sportsmot/test/", "").replace(
                            ".jpg", self.end_pkl_name
                        )

                path = os.path.join(self.public_det_path, pickle_name)
                pickle_res = pickle.load(open(path, "rb"))
                det_labels = torch.tensor(pickle_res["det_labels"]).to("cuda") if not isinstance(pickle_res["det_labels"], torch.Tensor) else pickle_res["det_labels"].to("cuda")
                det_bboxes = (
                    torch.tensor(pickle_res["det_bboxes"]).to("cuda").to(torch.float32) if not isinstance(pickle_res["det_bboxes"], torch.Tensor) else pickle_res["det_bboxes"].to("cuda").to(torch.float32)
                )
                if len(det_bboxes) != 0:
