...
class MOT17Metric(BaseVideoMetric):
    ...
    def evaluate(self, size: int = 1) -> dict:
        ...
        if is_main_process():
            ...
            result_files_path = f"{self.outfile_prefix}/mot17_track.json"
        ...
        # ====================================== wyy ======================================
            self.mot17_evaluation(all_seq_pred_json)

            metrics = [None]
        else:
            metrics = [None]  # type: ignore

        logger.info(f"Wait for all processes to complete evaluation.")
        # 阻塞所有线程，让主线程先完成评估再return（返回值会在MMDetection的库函数中被处理，会引发报错）
        barrier()
    ...
    # ====================================== wyy ======================================
    def mot17_evaluation(self, all_seq_pred_json):
        print("############################ Evaluation ############################")
        print(f"Start evaluation")

        # 数据集Annotations
        data_path = "data/mot/annotations/val_half.json"
        # 结果输出路径
        output_dir = f"{self.outfile_prefix}/mot17_half_tracking_results"
        # Masa的tracking结果
        tracking_data = all_seq_pred_json
        print(f"共包含{len(tracking_data)}条数据")

        with open(data_path, 'r') as f:
            dataset_info = json.load(f)

        img_info = dataset_info.get('images', [])
        video_info = dataset_info.get('videos', [])

        # 根据image_id到frame_id的映射
        id_to_frame_id = {entry['id']: entry['frame_id'] for entry in img_info}

        # 构建video_id到file_name映射表
        video_id_to_name = {}
        for item in video_info:
            video_id = item['id']
            file_name = item['file_name']
            video_id_to_name[video_id] = file_name

        os.makedirs(output_dir, exist_ok=True)

        # 根据Evaluation的格式要求将每个视频的跟踪结果保存为对应的txt文件
        for video_id in video_id_to_name:
            # 找到当前video_id对应的数据
            video_data = [entry for entry in tracking_data if entry['video_id'] == video_id]

            # 生成保存文件路径
            filename = f"{video_id_to_name[video_id]}.txt"
            ...
            with open(filepath, 'w') as f:
                for entry in video_data:
                    ...
                    # 根据image_id获取对应的frame_id
                    frame_id = id_to_frame_id.get(image_id, -1)  # 如果没有找到对应的frame_id，默认为-1

                    # 按照指定的格式写入txt文件，第一列是frame_id
                    f.write(
                        f"{frame_id}, {track_id}, {bbox[0]}, {bbox[1]}, {bbox[2]}, {bbox[3]}, {score}, -1, -1, -1\n")
        ...
        command = [
            ...
            '--GT_FOLDER', 'TrackEval/data/gt/MOT17-train',
            '--SEQMAP_FILE', 'TrackEval/data/gt/seqmaps/MOT17-train.txt',
            '--SKIP_SPLIT_FOL', 'True',
            '--TRACKERS_TO_EVAL', '',
            '--TRACKER_SUB_FOLDER', '',
            '--USE_PARALLEL', 'True',
            ...
            '--PLOT_CURVES', 'False',
            '--TRACKERS_FOLDER',
            'results/masa_results/mot17_half_masa-groundingdino-release_detic_dets-test/mot17_half_tracking_results'
        ]

        # 使用subprocess.run运行命令，完成评估
